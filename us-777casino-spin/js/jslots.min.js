...
(function ($) {
	$.jSlots = function (el, options) {
		...
		base.setup = function () {
			...
			base.increment =
				base.options.time / base.options.loops / base.options.loops;
			...
			base.$wrapper = $list
				.wrap('<div class="jSlots-wrapper"></div>')
				.parent();
		...
		base.bindEvents = function () {
			$(base.options.spinner).bind(base.options.spinEvent, function (
				event
			) {
				if (!base.isSpinning) {
					base.playSlots();
		...
		base.Slot.prototype = {
			...
			spinEm: function () {
				...
				that.$el.css("top", -base.listHeight).animate(
					{
						top: "0px",
					},
					that.spinSpeed,
			...
			finish: function () {
				...
				if (base.options.endNumbers.length > 0) {
					endNum =
						base.options.endNumbers[base.allSlots.indexOf(this)];
				} else {
		...
		base.checkWinner = function (endNum, slot) {
			...
			if (
				($.isArray(base.options.winnerNumber) &&
					base.options.winnerNumber.indexOf(endNum) > -1) ||
